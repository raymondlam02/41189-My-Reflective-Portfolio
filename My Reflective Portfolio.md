...
# What theory proved to be useful and why? What have I learnt from this?
I have learnt the reasons why people create models and simulations, the intended benefits and the importance of using multiple models. This weeks workshop has allowed me to realize the importance of modelling, and its prevalent presence in our everyday lives. Models are used everyday to predict actions, which allow for relevant changes or adjustments to be made to ensure society is working at its peak efficiency. I have also been introduced to many-model thinkers and how they are able to make smarter, more informed decisions by utilizing the data and information generated by models. 

## Week 3

# What have I learnt this week?
This week I learnt how to model human behavior; rational, behavioral and rule-based. 
...
Behavioral - this model uses a number of basis to more accurately depict human behavior, these basis are an attempt to factor in human irrationalities, bais' and general human behaviour. The behavioral model is based on observations of human psychology. 

Rule-based - this model uses psychology to make predictions about human behavior. Rule based models can be broken down into steps that humans take in order to accomplish a task. 
...
For decisions that have external uncertainties, probability models may be utilized in order to assist the decision maker visualize each outcome. Three types of probability models include;
...
Finally, agent based models were introduced this week. Agent based models are used to simulation the interactions between different agents, in an attempt to monitor and process the behaviors of each agent based on their relation to other agents. Agent based models are also able to comprehend the impact each of its agents has on the external environment. Agent based models have proven to be extremely useful to simulate societies response to and the subsequent effects of Covid-19.

Complex Systems vs Complicated Systems: 
Complex - Loosely connected, unpredictable and adaptive
Whilst complicated systems are - interconnected, dependent and predictable. 
...
This week I learnt about aggregating decisions and behaviors. Aggregation is often non-linear thus much more complicated than arithmetic aggregation. Aggregation studies the interactions between local components for new properties of behaviors. However, it is also known that individual agents or particles can behave differently when segregated. 
...
Probability is understanding the chances of a random outcome occurring from a random process. Although examples of probability are often used in gambling, probability is also prevalent in finance, weather forecasts and insurance firms. 

This week's workshop also included a number of concepts that I was already familiar as a student of Business Statistics, although the core concepts were familiar, the context and uses in which they are presented in this workshop were new to me.
A random variable is a collection of potential values, subject to random variations, each associated with a probability. 
...
Continuous random variables have an infinite number of possible outcomes. It is calculated by measuring the possibility that the variable is a number. 
...
In order to interpret data, a test for significance must be conducted. The test has two results, either there is enough information to either reject or not reject the hypothesis. If an empirical mean lies between the accepted means, a rejection of the hypothesis is made. 

Six Stigma
Six stigma method involves working to reduce the size of the standard deviation to lower the probability of a failure. Businesses and governments can do this by introducing some sort of quality control to ensure their plans or products stick to protocol. Six stigma reduces the standard deviation so that even a six-standard-deviation error avoids a malfunction.
Probability is valuable as it grants decision makers more information to entice more informed more efficient decisions. 

Long-tailed distributions model events that have a high probability of occurring. A 'Power Law' is a distribution which is top-heavy followed by a long tail. Long-tailed distributions are created through interdependence.
There are two models that give rise to power-distribution laws; the preferential attachment model and the self-organized critically model. 
Preferential Attachment Model 
A sequence of objects occur or arrive one after another. The first arrival creates an 'entity', with each subsequent arrival following the rule: with probability (p), the arrival forms a new entity, with probability (1-p) the arrival joins an existing entity. 

Self-organized Critically Model
The self-organized model produces a power-law distribution by building interdependencies in a system until which a time that the system reaches a critical state. At such time, any following events have often little to no effect on the overall system.
...
## Week 8
This week is a continuation to last weeks concepts, about complexity and emergent behavior. I learnt about the Colonel Blotto Game, the goal of the game is to win the war however in order to win the war one must win 10 minor fights. Each player has the same total amount of troops, with deployments of each fight dependent on the player. 
Although this is only a game, the applications of thinking derived from this game can be utilized in real-world events. 
Such as;
- US Elections
- Terrorism
- Trials
...
Prisoners' Dilemma is a thought experiment, which demonstrates game theory. Where two prisoners' are deprived from outside communication and encouraged to turn each other in, however;
- If none of the prisoners' confess they are both given a minimum sentence
- If one of the prisoners' confess and the other remains silent, the one that remained silent will face a maximum sentence
- If both of the prisoners' confess, they both get a lowered sentence
...
This week, pre-submissions for the assessment were due. Jordan, the leader of our group was extremely helpful as he made a messenger group chat which allowed for easy communication between group members and allowed everyone to stay accountable for their work. Progress before this week had been stagnant and minimal however this week everyone was able to get together and focus on their sections of the assignment. Working with my group so far has been easy and their efforts in the assignment have been extensive and of high quality. We were able to finish early and Jordan was able to compile our work from a Google document into the expected GitHub format. This also gave everyone the opportunity to edit and comment on each others work. As a result of this we were able to present a unified, coherent piece of work for our pre-submission.
Overall I was extremely satisfied with my groups work and their responses to comments and messages. 

Although I was pleased with the quality of our work, we did face a few challenges. Scheduling group calls or work sessions was quite hard due to the shift to online learning. Group members would often have class or external activities scheduled at different times, making it quite difficult to meet with the entire group. To combat this we picked the most suitable time for the majority of the group and made notes and communicated any relevant information to the members that could not make the meeting. 
Also, our group had trouble with Github as it was a new platform to us. We were able to reach out to other groups for assistance with formatting our document and the addition of images.
...
Jordan and I both believe we should've reached out to Juan earlier for assistance with Jupiter and GitHub as it would've saved us a tremendous amount of time and effort we otherwise wouldn't have had to waste. 

Finally, challenges include our group members not being able to make it to this weeks workshop. However, I believe this was out of their control, they had errands to run and family to attend to. I believe these were isolated, one-off incidents and that this will not happen in following weeks. Thus, overall I am still quite happy with my groups performance and look forward to working with my whole group next week. 
...
We decided to end our session early, as we agreed that our attention spans and focus was coming to an end after almost of the 3 hours of workshop. We scheduled plans for a future group meeting to check up on each others progress and answer any questions any of us may have. Due to the nature of this task, pervious sections have to be completed in an ordered sequence thus we encouraged members working on the first parts of the assignment. Looking back however I feel it would've been more productive and efficient if we had stayed in call and worked on our assessment for a little while longer. It was likely most of our group procrastinated after the workshop and as we had assessments and exams due for other subjects we should've made more use of the allocated time to work on our assessment. 

## Week 11

This week, again Juan gave us the opportunity to work on our assignments in preparation for the final submission date. Our group continued to work on the assignment whilst in contact with Juan for assistance with JupiterNotebook and Python coding. As our whole group was unfamiliar with Python, the code and its subsequent actions made little to no sense to us. We were encouraged to use a pre-existing piece of code then modify as required. However, we were having issues running the pre-existing code. Juan read through the code, however we asked him towards the end of the workshop. He was able to get back to us in the following days and we were able to start using Python to model the effects of Covid-19 as required for the task. I found this week's workshop extremely productive for our group as we were able to achieve a lot, although it may not be evident immediately as we were not able to materialize some of the concepts and ideas during that workshop, we discussed ideas and ways we could answer each question. I feel this weeks QnA session in the workshop was extremely valuable to our group as we were able to realize what each question was asking. This will hopefully result in a better performance in the final assessment. 


## Week 12

This is the final week of the sesmester, Juan again allowed us to work on our assessment to be due on the 6th of November. The extenstion he offered us allowed me to prioritize other subjects that were due first and eventually focus all my energy into this final task. During this week my group and I looked more into the agent-based models that were avaliable for us to use in our assignment. With a specific agent-based Covid 19 model catching out eye, we set off to try customize the code to suit our situation. However, again we faced troubles with Python code and the JupiterNotebook platform. However, with the model we chose we were able to visualise the effects of each decision made by governments and the subsequent reaction from citizen interactions. As mentioned in our prior submission we wanted to not only model the spread of the virus but also simulate the reactions of citizens and to government intervention. In an effort to investigate the start of the Melbourne Covid-19 Riots and similar riots that have been happening across the world. 
